...
<template>
    <div class="top_up_record pt30">
        <table class="data_list p_100 tc">
            <thead><tr><th>充值金额</th><th>充值方式</th><th>充值时间</th></tr></thead>
            <tbody>
                <tr v-for="item in topUpData.slice((currentPage-1) * pageSize,currentPage * pageSize)">
                    <td>￥{{item.recharge_amount}}</td>
                    <td>{{pay_style[item.recharge_style]}}</td>
                    <td>{{item.add_time}}</td>
                </tr>
            </tbody>
        </table>
